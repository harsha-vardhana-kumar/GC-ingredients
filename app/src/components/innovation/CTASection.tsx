  ...
  return (
    <section className="py-24 bg-[#0d3a22] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <motion.div className="flex justify-center mb-6" initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}>
          <div className="w-16 h-16 rounded-2xl bg-[#e8650a]/20 flex items-center justify-center">
            <Lightbulb className="w-8 h-8 text-[#e8650a]" />
          </div>
        </motion.div>
        ...
        <motion.h2 className="text-4xl lg:text-5xl font-bold font-[family-name:var(--font-plus-jakarta)] text-white leading-[1.15] mb-5 max-w-3xl mx-auto" variants={fadeUp(0.1)} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
          Have an Ingredient Challenge?{" "}
          <span className="text-[#e8650a]">Let&apos;s Solve It.</span>
        </motion.h2>
        <motion.p className="text-white/70 text-lg max-w-2xl mx-auto mb-10 leading-relaxed" variants={fadeUp(0.2)} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
          Our food scientists are ready to tackle your toughest formulation challenges — from a simple ingredient swap to a full custom blend development project.
        </motion.p>
        <motion.div className="flex flex-wrap gap-4 justify-center" variants={fadeUp(0.3)} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
          ...
          <a href="#" className="inline-flex items-center gap-2 border-2 border-white text-white font-semibold px-8 py-4 rounded-xl hover:bg-white hover:text-[#0d3a22] transition-all hover:-translate-y-0.5">
            <Download className="w-4 h-4" /> Download Innovation Brief
          </a>
        </motion.div>
      </div>
    </section>
