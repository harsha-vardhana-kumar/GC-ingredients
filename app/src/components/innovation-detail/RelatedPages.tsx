...
export default function RelatedPages({ pages }: RelatedPagesProps) {
  return (
    <section
      ...
      aria-label="Related Innovation Pages"
    >
          ...
          {pages.map((page, i) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-[#1a5c38]/8 border border-[#1a5c38]/10 flex items-center justify-center group-hover:bg-[#1a5c38] transition-colors duration-300">
                <GCIcon name={page.Icon} className="w-6 h-6 text-[#1a5c38] group-hover:text-white transition-colors duration-300" />
              </div>
              <div>
                ...
                <p className="text-[#6b7280] text-sm leading-relaxed">{page.description}</p>
              </div>
