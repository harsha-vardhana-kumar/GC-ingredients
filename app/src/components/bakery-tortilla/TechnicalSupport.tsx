...
const supportPoints = [
  "Free initial formulation consultation",
    ...
    <section className="py-20 bg-[#1a5c38] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Left */}
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.75, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <h2 className="text-3xl lg:text-4xl font-bold font-[family-name:var(--font-plus-jakarta)] text-white leading-[1.2] mb-5">
              Our Food Scientists Work <span className="text-[#e8650a]">Alongside Yours</span>
            </h2>
            <p className="text-white/70 leading-relaxed mb-8 text-[17px]">
              From first consultation to final production — our bakery and tortilla ingredient specialists are with you every step of the way.
            </p>
            <div className="space-y-3 mb-8">
              {supportPoints.map((pt, i) => (
                <motion.div key={pt} className="flex items-center gap-3" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: 0.2 + i * 0.1 }}>
          ...
          <div className="grid grid-cols-3 divide-x divide-white/20">
            {stats.map((s, i) => (
              <motion.div key={s.label} className="text-center px-4 py-6" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.55, delay: i * 0.15 }}>
                <div className="text-3xl font-bold font-[family-name:var(--font-plus-jakarta)] text-[#e8650a] mb-2">{s.value}</div>
